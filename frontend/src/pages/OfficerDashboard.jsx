...
export default function OfficerDashboard() {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1e293b', marginBottom: 24 }}>{t('officer.commandCenter')}</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        <div style={{ background: '#fff', padding: 20, borderRadius: 8, border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            <div style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 600 }}>{t('officer.criticalRiskAreas')}</div>
        ...
        <div style={{ background: '#fff', padding: 20, borderRadius: 8, border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            <div style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 600 }}>{t('officer.activeAdvisories')}</div>
        ...
        <div style={{ background: '#fff', padding: 20, borderRadius: 8, border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            <div style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 600 }}>{t('officer.farmersReached')}</div>
      ...
      <div style={{ background: '#fff', padding: 24, borderRadius: 8, border: '1px solid #e2e8f0' }}>
        <h2 style={{ fontSize: '1rem', fontWeight: 600, color: '#334155', marginBottom: 16 }}>{t('officer.priorityAction')}</h2>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.9rem' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #f1f5f9' }}>
              <th style={{ padding: '12px 8px', color: '#64748b' }}>{t('officer.region')}</th>
              <th style={{ padding: '12px 8px', color: '#64748b' }}>{t('officer.primaryRisk')}</th>
              <th style={{ padding: '12px 8px', color: '#64748b' }}>{t('officer.lastAdvisory')}</th>
              <th style={{ padding: '12px 8px', color: '#64748b' }}>{t('officer.action')}</th>
            </tr>
          </thead>
          <tbody>
            <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
              ...
              <td style={{ padding: '12px 8px' }}><button style={{ background: '#1e293b', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: 4, cursor: 'pointer' }}>{t('officer.review')}</button></td>
            </tr>
            <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
