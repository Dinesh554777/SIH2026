...
function installApi({
  ...
  const handler = (input) => {
    ...
    if (path === "/api/v1/demo/cells-risk") return Promise.resolve(ok(cellsRiskFixture));
    const m = path.match(
...
describe("Hyperlocal Monsoon Decision Support frontend", () => {
  ...
  it("renders the risk legend with whole-grid counts and date label", async () => {
    installApi();
    renderApp();
    await screen.findByText(/Dry spell probability is 92%/i);
    const legend = await screen.findByTestId("risk-legend");
    expect(legend).toHaveTextContent("Risk level (whole grid)");
    expect(legend).toHaveTextContent("Critical");
    expect(legend).toHaveTextContent("derived from frozen model probabilities");
    expect(legend).toHaveTextContent("2024-08-12");
  });

  it("lays out the forecast timeline: past, current model read, and an honest future note", async () => {
    installApi();
    renderApp();
    await screen.findByText(/Dry spell probability is 92%/i);
    const tl = await screen.findByTestId("forecast-timeline");
    const future = within(tl).getByTestId("timeline-future");
    expect(future).toHaveTextContent(/observation date only/i);
    expect(within(future).getAllByText(/not available/i).length).toBeGreaterThanOrEqual(3);
    const past = within(tl).getByTestId("timeline-past");
    expect(past).toHaveTextContent("26 day(s)");
    expect(past).toHaveTextContent("2.97 mm");
    expect(within(tl).getByTestId("timeline-current")).toHaveTextContent("91% · very_high");
  });

  it("renders three risk cards with honest windows and actions", async () => {
    installApi();
    renderApp();
    await screen.findByText(/Dry spell probability is 92%/i);
    expect(await screen.findByTestId("risk-card-false-onset")).toHaveTextContent("low");
    expect(screen.getByTestId("risk-card-dry-spell")).toHaveTextContent("26 day(s) dry so far");
    expect(screen.getByTestId("risk-card-dry-spell")).toHaveTextContent("92% · high");
    expect(screen.getByTestId("risk-card-break")).toHaveTextContent("91% · high");
    expect(screen.getAllByTestId("risk-cards")).toHaveLength(1);
  });

  it("marks the action centre heading and location recommendation", async () => {
    installApi();
    renderApp();
    await screen.findByText(/Dry spell probability is 92%/i);
    expect(screen.getByRole("heading", { name: /WHAT SHOULD I DO NOW/i })).toBeInTheDocument();
    expect(screen.getByTestId("action-badge")).toHaveTextContent("Irrigation prepare");
  });
});